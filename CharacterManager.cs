...
public class CharacterManager : BaseManager<CharacterManager>
{
    ...
    public void RemoveCharacter(GameObject character)
    {
        ...
        if (gameCharacter.TryGetComponent<BaseAI>(out var beforeAI))
            GameObject.Destroy(beforeAI);

        ObjectPoolManager.Instance.ReleaseObject(character);
    }
    ...
    public List<GameCharacter> GetEnemiesInRange(GameCharacter myCharacter, Rect rangeRect, int target = 1)
    {
        ...
        rangeRect.position = myCharacter.TRANSFORM.position;
